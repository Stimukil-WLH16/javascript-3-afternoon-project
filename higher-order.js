...
/*
  Use a higher order method to get all the order totals after adding in the sales tax (given to you as a tax rate, hint: 
    you'll need to do some multiplication===> price * (1 + tax) <===). Your answer should be an array of numbers, one total for each order.
*/

let orderTotals = orders.reduce(function(element, index, array) {
  if(index === orders.length - 1) {
    return acc + (element['price'] * (1 + element['tax']))
  }
}, [])
